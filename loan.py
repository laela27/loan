...
st.write("Model Random Forest menunjukkan kinerja terbaik di semua metrik dibandingkan dengan model lain, namun kinerja di data validation mengalami penurunan yang cukup tinggi dibandingakan data train maka perlu dilakukan tuning untuk menghindari overfitiing")


st.write("**c.2.3 Tuning Hyperparamater**")
...
y_val_pred_proba_rf = best_rf_model.predict_proba(X_val)[:, 1]

y_test_pred_rf = best_rf_model.predict(X_test)
y_test_pred_proba_rf = best_rf_model.predict_proba(X_test)[:, 1]

# Evaluasi model pada data training
st.write("**Evaluasi Model Random Forest pada Data Training**")
st.text("Classification Report:\n" + classification_report(y_train, y_train_pred_rf))
st.text("Akurasi: {:.4f}".format(accuracy_score(y_train, y_train_pred_rf)))
st.text("AUC-ROC: {:.4f}".format(roc_auc_score(y_train, y_train_pred_proba_rf)))
st.text("F1-Score: {:.4f}".format(f1_score(y_train, y_train_pred_rf)))
...
st.write("**Evaluasi Model Random Forest pada Data Testing**")
st.text("Classification Report:\n" + classification_report(y_test, y_test_pred_rf))
st.text("Akurasi: {:.4f}".format(accuracy_score(y_test, y_test_pred_rf)))
st.text("AUC-ROC: {:.4f}".format(roc_auc_score(y_test, y_test_pred_proba_rf)))
st.text("F1-Score: {:.4f}".format(f1_score(y_test, y_test_pred_rf)))
...
# Menampilkan hasil model Random Forest setelah tuning
st.write("Model **Random Forest** dengan tuning menunjukkan hasil yang lebih baik dari sebelum di-tuning, khususnya pada nilai **F1-Score** yang meningkat dari sebelumnya.")
...
# Menghitung confusion matrix untuk Random Forest pada data test
conf_matrix = confusion_matrix(y_test, y_test_pred_rf)
...
st.write("**Hasil Prediksi Loan Approval menggunakan model Random Forest**")
st.write("**True Positives (TP):** 500 pinjaman yang seharusnya disetujui dan benar-benar disetujui.")
st.write("**True Negatives (TN):** 25 pinjaman yang seharusnya ditolak dan benar-benar ditolak.")
st.write("**False Positives (FP):** 296 pinjaman yang salah diprediksi sebagai disetujui.")
st.write("**False Negatives (FN):** 28 pinjaman yang salah diprediksi sebagai ditolak.")
...
# Feature Importance
# Visualisasi feature importance di Streamlit
st.write("**c.2.4 Feature Importance tanpa CIBIL score dari Model Random Forest yang di tuning**")
...
plt.figure(figsize=(20, 16))  # Meningkatkan ukuran plot lebih besar
barplot = sns.barplot(x='Importance', y='Feature', data=importance_df, color='blue')  # Mengubah warna batang menjadi biru

# Menambahkan label di atas setiap batang dengan 2 angka di belakang koma
for index, value in enumerate(importance_df['Importance']):
    plt.text(value + 0.002, index, f'{value:.2f}', va='center', fontsize=16)  # Menambahkan angka di atas batang dengan font lebih besar

# Menambahkan judul dan label sumbu
plt.title('Feature Importance dari Model Random Forest setelah Tuning', fontsize=28)  # Ukuran judul lebih besar
plt.xlabel('Mean Importance', fontsize=24)  # Ukuran label sumbu X lebih besar
plt.ylabel('Feature', fontsize=24)  # Ukuran label sumbu Y lebih besar

# Mengatur ukuran label di sumbu X dan Y agar lebih besar
plt.xticks(fontsize=20)  # Ukuran label sumbu X lebih besar
plt.yticks(fontsize=20)  # Ukuran label sumbu Y lebih besar

# Mengatur layout agar tidak ada yang terpotong
plt.tight_layout(pad=3.0)  # Meningkatkan padding agar ada ruang lebih antara elemen
...
st.write("Diikuti oleh **loan to value ratio** (importance: **0,21**) dan **loan term** (importance: **0,20**).")
st.write("Fitur lainnya seperti **income annum**, **dependent to income ratio**, dan **education** memberikan kontribusi yang lebih kecil dengan importance di bawah **0,10**.")
...
st.write("- **Dengan CIBIL score**, model menunjukkan ketergantungan tinggi pada fitur ini, yang memiliki **importance tertinggi**.")
st.write("- **Tanpa CIBIL score**, **Debt to Income Ratio** menjadi fitur terpenting, diikuti oleh **Loan to Value Ratio** dan **Loan Term**.")
